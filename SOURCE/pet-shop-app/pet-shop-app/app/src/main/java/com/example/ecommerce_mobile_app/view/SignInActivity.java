...
public class SignInActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        activitySignInBinding.tvForgotpassword.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
            ...
            public void onResponse(@NonNull Call<LoginResponse> call, @NonNull Response<LoginResponse> response) {
                if (response.isSuccessful()){
                    if (response.body() != null && Constant.STATUS_SUCCESS.equals(response.body().getHttpStatus())) {
                        ...
                        CustomToast.showSuccessMessage(getApplicationContext(), "Login successful!");
                        prefManager.saveLoginUser(customer);
                        prefManager.saveToken(response.body().getToken());
                        startMainActivity();
                    } if (StringUtils.isEmpty(username) && StringUtils.isEmpty(password)) {
                        CustomToast.showFailMessage(getApplicationContext(), "Please enter in full");
                    } if(StringUtils.isBlank(username)){
                        CustomToast.showSuccessMessage(getApplicationContext(), "Please enter your Username");
                    } if(StringUtils.isBlank(password)) {
                        CustomToast.showSuccessMessage(getApplicationContext(), "Please enter your Password");
                    }else{
                        CustomToast.showSuccessMessage(getApplicationContext(), "Incorrect username or password");
                    }
                }
            }
